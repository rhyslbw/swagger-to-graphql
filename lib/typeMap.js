...
var sanitizeGraphQLTypeName = function sanitizeGraphQLTypeName(typeName) {
  return typeName.replace(/-/g, '');
};

var getExistingType = function getExistingType(ref, isInputType, gqlTypes) {
  var refTypeName = getTypeNameFromRef(ref);
  var typeName = sanitizeGraphQLTypeName(refTypeName);
...
var getTypeFields = exports.getTypeFields = function getTypeFields(jsonSchema, title, isInputType, gqlTypes) {
  ...
  return function () {
    return _lodash2.default.mapKeys(_lodash2.default.mapValues(jsonSchema.properties || {}, function (propertySchema, propertyName) {
      var baseType = jsonSchemaTypeToGraphQL(title, propertySchema, propertyName, isInputType, gqlTypes);
      var type = jsonSchema.required && jsonSchema.required.includes(propertyName) && !isObjectType(jsonSchema.properties[propertyName]) ? graphql.GraphQLNonNull(baseType) : baseType;
      return {
